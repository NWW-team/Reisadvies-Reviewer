# Bulkrapport reisadviezen

Gedraaid op 2026-09-15 over 226 reisadviezen.

## Hoe groot is de achterstand

- **154** van de 226 adviezen hebben geen enkele harde fout.
- **46** adviezen zitten boven de woordenlimiet.
- Gemiddeld 0.4 eigen fouten per advies, los van wat uit de standaardtekst komt.
- **88** van de 5214 bevindingen steunen op een aanvulling: de regel komt uit de schrijfwijzer, maar het woord of domein staat er niet letterlijk in. Die zijn in de tool gemarkeerd.

## Zit in de standaardtekst

Deze bevindingen staan woordelijk gelijk in 45 adviezen of meer. Eén keer aanpassen in
de standaardtekst lost ze allemaal tegelijk op.

| regel | in hoeveel adviezen | wat |
|---|---:|---|
| `percentage-notatie` | 223 | "100 procent" moet als 5% — symbool, zonder spatie. |
| `link-tekstlengte` | 221 | Linktekst is 74 tekens. Maximaal 70. |
| `zin-max-woorden` | 218 | Deze zin telt 17 woorden. Maximaal 15. De linktekst telt mee — kort die eerst in. |
| `zin-max-woorden` | 212 | Deze zin telt 17 woorden. Maximaal 15. De linktekst telt mee — kort die eerst in. |
| `zin-max-woorden` | 209 | Deze zin telt 21 woorden. Maximaal 15. |
| `zin-lijdende-vorm` | 208 | Lijdende vorm: "wordt … beroofd". Schrijf actief. |
| `link-tekstlengte` | 208 | Linktekst is 75 tekens. Maximaal 70. |
| `zin-max-woorden` | 192 | Deze zin telt 32 woorden. Maximaal 15. De linktekst telt mee — kort die eerst in. |
| `zin-max-woorden` | 191 | Deze zin telt 17 woorden. Maximaal 15. De linktekst telt mee — kort die eerst in. |
| `zin-max-woorden` | 179 | Deze zin telt 16 woorden. Maximaal 15. De linktekst telt mee — kort die eerst in. |
| `zin-max-woorden` | 47 | Deze zin telt 18 woorden. Maximaal 15. De linktekst telt mee — kort die eerst in. |

## Per regel

| regel | adviezen | bevindingen |
|---|---:|---:|
| `zin-max-woorden` | 226 | 3150 |
| `link-tekstlengte` | 226 | 486 |
| `percentage-notatie` | 224 | 224 |
| `zin-lijdende-vorm` | 218 | 483 |
| `zin-twijfeltaal` | 183 | 527 |
| `kleur-eerste-bullet-voluit` | 68 | 68 |
| `aanhalingstekens` | 53 | 87 |
| `doc-woordenaantal` | 46 | 46 |
| `kleur-aanduiding` | 33 | 34 |
| `h3-niet-melden` | 25 | 25 |
| `tussenkop-leestekens` | 11 | 11 |
| `vragen-opeenvolgend` | 10 | 11 |
| `actueel-max-woorden` | 10 | 10 |
| `link-verboden-partij` | 6 | 7 |
| `opmaak-vet-onderstreept` | 5 | 5 |
| `eenheden-voluit` | 5 | 5 |
| `titel-leestekens` | 5 | 5 |
| `h2-vast` | 4 | 4 |
| `schuine-streep` | 4 | 4 |
| `afkortingen-uitschrijven` | 3 | 3 |
| `lhbtiq-schrijfwijze` | 2 | 2 |
| `kleur-vaste-tekst` | 2 | 2 |
| `alinea-max-woorden` | 2 | 4 |
| `getallen-cijfers` | 2 | 2 |
| `telefoon-notatie` | 2 | 3 |
| `genderneutraal` | 2 | 3 |
| `kort-informatieservice` | 1 | 1 |
| `valuta-notatie` | 1 | 1 |
| `tijd-notatie` | 1 | 1 |

## Langste adviezen

| land | woorden | limiet |
|---|---:|---:|
| Thailand | 3330 | 2000 |
| India | 3100 | 2000 |
| Ecuador | 2754 | 2000 |
| Indonesië | 2594 | 2000 |
| Colombia | 2556 | 2000 |
| Turkije | 2539 | 2000 |
| China | 2504 | 1500 |
| Rusland | 2501 | 2000 |
| Mexico | 2497 | 2000 |
| Peru | 2460 | 2000 |
| Tanzania | 2438 | 2000 |
| Filipijnen | 2436 | 2000 |
| Venezuela | 2332 | 2000 |
| Egypte | 2326 | 2000 |
| Irak | 2283 | 2000 |

---

Dit rapport gaat alleen over objectief toetsbare regels. Politieke gevoeligheden staan er
bewust niet in, en de tool kort niets in: bij een overschrijding wijst hij de langste blokken
aan, maar wat weg kan is een inhoudelijke keuze.
