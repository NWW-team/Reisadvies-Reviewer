# Erheen of hierheen

De matrix, tabblad *Kleurcode-teksten*, kolom *In het kort*, rijen Groen en Geel:

> **Volledig geel:** De kleurcode van het reisadvies voor land X is geel. U kunt **erheen** reizen. …
>
> **Deels geel:** Voor de gebieden X en Y/de rest van land X geldt kleurcode geel. U kunt **hierheen** reizen. …

Welke van de twee het is, hangt aan het aantal kleurcodes van het advies: één kleurcode betekent
het hele land, meer kleurcodes betekent dat elke bullet over een deel gaat.

Over 226 reisadviezen staat dit 20 keer andersom. Eén woord per advies.

| land | wat er staat | moet zijn |
|---|---|---|
| Verenigde Arabische Emiraten | De kleurcode van het reisadvies voor de VAE is geel. U kunt erheen reizen. | hierheen |
| Botswana | De kleurcode van het reisadvies voor Botswana is geel. U kunt hierheen reizen. | erheen |
| Ivoorkust | Voor de rest van Ivoorkust geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Comoren | De kleurcode van het reisadvies voor de Comoren is geel. U kunt hierheen reizen. | erheen |
| Ecuador | Voor de rest van Ecuador geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Georgië | Voor de rest van Georgië geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| IJsland | De kleurcode van het reisadvies is geel voor het schiereiland Reykjanes. U kunt erheen reizen. | hierheen |
| IJsland | Voor de rest van IJsland geldt kleurcode groen. U kunt erheen reizen. | hierheen |
| Cambodja | Voor de rest van Cambodja geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Marokko | Vor de rest van Marokko geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Moldavië | Voor de rest van Moldavië geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Oman | Voor de rest van Oman geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Peru | Voor de rest van Peru geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Palestijnse Gebieden | Voor Oost-Jeruzalem geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Roemenië | Voor de rest van Roemenië geldt kleurcode groen. U kunt erheen reizen. | hierheen |
| Rwanda | Voor de rest van Rwanda geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| Saoedi-Arabië | Voor de rest van Saoedi-Arabië geldt kleurcode geel. U kunt erheen reizen. | hierheen |
| El Salvador | De kleurcode van het reisadvies voor El Salvador is geel. U kunt hierheen reizen. | erheen |
| Turks- en Caicoseilanden | De kleurcode van het reisadvies is geel voor het eiland Providenciales. U kunt erheen reizen. | hierheen |
| Turks- en Caicoseilanden | Voor de rest van de Turks- en Caicoseilanden geldt kleurcode groen. U kunt erheen reizen. | hierheen |

Gegenereerd met `node scripts/erheen-hierheen.mjs` op 2026-09-19.
