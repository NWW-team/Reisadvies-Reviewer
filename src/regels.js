...
export function maakToetser(data) {
  ...
  return function toets(doc, opties = {}) {
    ...
    for (const kleur of kleurenVanAdvies) {
      ...
      //
      // Maar ook bij meerdere kleurcodes gaat niet elke bullet over een afgebakend deelgebied. Wijst
      // hij terug naar iets specifiek dat eerder in de zin genoemd is ("voor de gebieden X en Y"),
      // dan is dat een deelgebied en hoort "hierheen" erbij. Gaat het om een groter, abstracter
      // geheel -- "voor de rest van land X", of "het hele land" -- dan is dat geen gerichte
      // verwijzing en hoort ook daar "erheen" bij, ook al heeft het advies als geheel meerdere
      // kleurcodes. Dat onderscheid zit in de bullet zelf, niet in het aantal kleuren van het advies.
      //
      ...
      const opening = bullet ? norm(bullet.split(/(?<=\.)\s+/)[0]) : '';
      const wijstNaarGeheel = kleurenVanAdvies.length > 1 && /\b(?:de rest van|het hele)\b/.test(opening);
      const hoortVolledig = kleurenVanAdvies.length === 1 || wijstNaarGeheel;
      ...
      if (instructieVan(verwachteTekst) && !kortGenorm.includes(instructieVan(verwachteTekst))) {
        ...
        b.push(bevinding('kleur-variant', ERNST.fout, 'MX, tab Kleurcode-teksten',
          staatDeAndere
            ? (hoortVolledig
                ? `Deze bullet gaat over het hele land of de rest ervan, dus bij ${kleur} hoort de volledige uitleg. Nu staat de verkorte variant er.${preciezer}`
                : `Deze bullet wijst een deelgebied aan, dus bij ${kleur} hoort de verkorte variant. Nu staat de volledige uitleg er.${preciezer}`)
          ...
          { ...(bullet ? { fragment: bullet } : {}),
            ...
            notitie: hoortVolledig
              ? 'Bij het hele land, of de rest ervan, geldt de kleur voor een groter geheel; dan staat de volledige uitleg in "In het kort".'
              : 'Bij een gericht deelgebied staat de verkorte uitleg; de volledige uitleg volgt onder Regionale risico\'s.' }));
      }
    }
